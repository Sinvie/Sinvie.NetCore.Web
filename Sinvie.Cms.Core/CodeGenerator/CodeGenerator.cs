...
namespace Sinvie.Cms.Core.CodeGenerator {
    /// <summary>
    /// sinvie
    /// 2020.03.05
    ...
    public class CodeGenerator {
        ...
        private void GenerateIServices(DbTable table, bool ifExsitedCovered = true) {
            var iServicesPath = _options.OutputPath + Delimiter + _options.IServicesNamespace;
        ...
        private void GenerateServices(DbTable table, bool ifExsitedCovered = true) {
            var repositoryPath = _options.OutputPath + Delimiter + _options.ServicesNamespace;
        ...
        private void GenerateIRepository(DbTable table, string keyTypeName, bool ifExsitedCovered = true) {
            var iRepositoryPath = _options.OutputPath + Delimiter + _options.IRepositoryNamespace;
        ...
        private void GenerateRepository(DbTable table, string keyTypeName, bool ifExsitedCovered = true) {
            var repositoryPath = _options.OutputPath + Delimiter + _options.RepositoryNamespace;
            if (!Directory.Exists(repositoryPath)) {
                Directory.CreateDirectory(repositoryPath);
            }
        ...
        private void GenerateModelpath(DbTable table, out string path, out string pathP) {
            ...
            var modelPath = _options.OutputPath + Delimiter + _options.ModelsNamespace;//D:\\Sinvie.Cms.Web\Sinvie.Cms.Models
            if (!Directory.Exists(modelPath)) {
                Directory.CreateDirectory(modelPath);
            }
            path = modelPath + Delimiter + table.TableName + ".cs";
            ...
            pathP = fullPath.ToString() + Delimiter + table.TableName + ".cs";
        }
        ...
        private string ReadTemplate(string templateName) {
            ...
            var strName = $"{currentAssembly.GetName().Name}.CodeTemplate.{templateName}";
            using (var stream = currentAssembly.GetManifestResourceStream(strName)) {
                if (stream != null) {
